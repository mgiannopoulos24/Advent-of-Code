...
// Function to process the rearrangement instructions with CrateMover 9001 (moves crates in bulk while retaining order)
function processInstructions(lines, stacks) {
    for (let line of lines) {
        if (line.startsWith('move')) {
            const [_, quantity, from, to] = line.match(/move (\d+) from (\d+) to (\d+)/).map(Number);

            // Remove crates in bulk from the 'from' stack
            const crates = stacks[from - 1].splice(-quantity);
            // Move them to the 'to' stack, retaining their order
