...
function solveBlueprints(input, timeLimit = 32) {
    const blueprints = parseBlueprints(input);
    
    // Take only the first 3 blueprints
    const firstThreeBlueprints = blueprints.slice(0, 3);
    
    const maxGeodes = firstThreeBlueprints.map(blueprint => {
        return simulateBlueprint(blueprint, timeLimit);
    });

    console.log('Max Geodes per Blueprint:', maxGeodes);
    
    return maxGeodes.reduce((a, b) => a * b, 1);
}
...
fs.readFile('input_level_19.txt', 'utf8', (err, data) => {
    ...
    console.log('Product of Max Geodes:', result);
});