...
// Solve part B: Find the first round where no elves move
function solvePartB(input) {
    ...
    while (true) {
        ...
        // If no elves moved, return the current round + 1 
        // (since rounds are 0-indexed but problem asks for round number)
        if (!result.moved) {
            return round + 1;
        }
...
function main() {
    ...
    const firstRoundNoMoves = solvePartB(input);
    console.log(`First round where no Elf moves: ${firstRoundNoMoves}`);
}
