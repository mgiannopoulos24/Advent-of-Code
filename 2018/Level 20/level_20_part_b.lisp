...
(let* ((regex (read-input "input_level_20.txt"))
  ...
  ;; Part 1: Find the furthest room
  (let ((furthest-room (loop for dist being the hash-values of visited-rooms
                            maximize dist)))
    (format t "The largest number of doors required to reach a room is: ~A~%" 
            furthest-room))
  
  ;; Part 2: Count rooms with at least 1000 doors
  (let ((rooms-at-least-1000 
         (loop for dist being the hash-values of visited-rooms
               count (>= dist 1000))))
    (format t "The number of rooms that require passing through at least 1000 doors is: ~A~%"
            rooms-at-least-1000)))