def parse_input(file_name):
    ...
    # Ensure there are no blank lines or incorrect formatting
    if len(lines) < 3:
        raise ValueError("Input file is incorrectly formatted.")
    
    enhancement_algorithm = lines[0]
    image = [list(line) for line in lines[2:]]  # Skip the empty line and read the image part
...
def process_image(file_name, steps):
    ...
    # Initial background value for infinite image region (assumed to be dark by default)
    default_char = '.'
    
    for step in range(steps):
        # Pad the image with the default char (dark pixels or light pixels depending on the toggle)
        image = pad_image(image, default_char, pad_size=2)
        
        # Enhance the image
        image = enhance_image(image, enhancement_algorithm, default_char)
        
        # Toggle the infinite background if the first char in the algorithm is '#'
...
if __name__ == "__main__":
    result = process_image("input_level_20.txt", 50)
    print(f"Number of lit pixels after 50 enhancements: {result}")
