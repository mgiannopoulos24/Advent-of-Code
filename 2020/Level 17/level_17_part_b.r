...
# Parse the initial 2D slice as a set of active cubes in 4D space (x, y, z, w)
active_cubes <- list()
...
# Function to get neighbors of a given cube in 4D
get_neighbors <- function(x, y, z, w) {
  neighbors <- list()
  for (dx in -1:1) {
    for (dy in -1:1) {
      for (dz in -1:1) {
        for (dw in -1:1) {
          if (!(dx == 0 && dy == 0 && dz == 0 && dw == 0)) {
            neighbors[[paste(x + dx, y + dy, z + dz, w + dw, sep = ",")]] <- TRUE
          }
        }
      }
    }
...
# Simulate one cycle in 4D space
simulate_cycle <- function(active_cubes) {
  ...
  for (cube in names(active_cubes)) {
    ...
    neighbors <- get_neighbors(coords[1], coords[2], coords[3], coords[4])
