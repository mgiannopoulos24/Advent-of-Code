...
int happiness[MAX_PEOPLE + 1][MAX_PEOPLE + 1] = {0};  // Extra space for "You"
char names[MAX_PEOPLE + 1][MAX_NAME_LENGTH];  // Extra space for "You"
int num_names = 0;
int max_happiness = -1000000;
int optimal_arrangement[MAX_PEOPLE + 1];

// Find the index of a name, or add it to the list if not found
...
// Calculate the total happiness for a given arrangement
...
// Read input from a file and populate the happiness matrix
void read_input() {
    const char *filename = "input_level_13.txt";
...
// Add "You" to the list and set neutral happiness with everyone
void add_yourself() {
    int my_index = find_name_index("You");
    for (int i = 0; i < num_names; i++) {
        if (strcmp(names[i], "You") != 0) {
            happiness[my_index][i] = 0;
            happiness[i][my_index] = 0;
        }
    }
}
...
// Find all possible arrangements of the people to maximize happiness
...
int main() {
    ...
    printf("The optimal arrangement, including yourself, has a total happiness of %d.\n", max_happiness);
