...
int matches(AuntSue aunt, int detected[]) {
    ...
    if (aunt.cats != -1 && aunt.cats != detected[1]) return 0;
    if (aunt.samoyeds != -1 && aunt.samoyeds != detected[2]) return 0;
    if (aunt.pomeranians != -1 && aunt.pomeranians != detected[3]) return 0;
    if (aunt.akitas != -1 && aunt.akitas != detected[4]) return 0;
    if (aunt.vizslas != -1 && aunt.vizslas != detected[5]) return 0;
    if (aunt.goldfish != -1 && aunt.goldfish != detected[6]) return 0;
    if (aunt.trees != -1 && aunt.trees != detected[7]) return 0;
