...
// Function to compute the decompressed length recursively
long long computeDecompressedLength(const string& input) {
    ...
    while (i < input.size()) {
        if (input[i] == '(') {
            ...
            // Recursively compute the length of the section to be repeated
            long long sectionLength = computeDecompressedLength(input.substr(i, lengthToRepeat));
            length += sectionLength * repeatTimes;
