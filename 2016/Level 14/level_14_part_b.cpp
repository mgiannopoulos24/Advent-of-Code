...
// Function to compute stretched MD5 hash
...
int main() {
    ...
    while (keys_found < target_keys) {
        // Generate stretched hash for the current index if not already generated
        if (hashes.find(index) == hashes.end()) {
            hashes[index] = stretched_md5(salt + std::to_string(index));
        }

        // Check for a triplet in the current hash
        char triplet_char = find_triplet(hashes[index]);
        if (triplet_char != '\0') {
            // Precompute the next 1000 stretched hashes if not already computed
            for (int i = index + 1; i <= index + 1000; ++i) {
                if (hashes.find(i) == hashes.end()) {
                    hashes[i] = stretched_md5(salt + std::to_string(i));
                }
            }
